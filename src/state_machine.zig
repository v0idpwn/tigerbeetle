        ...
        fn create_transfer(self: *StateMachine, t: *const Transfer) CreateTransferResult {
            ...
            const amount = amount: {
                ...
                if (t.flags.debits_at_most) {
                    const dr_limit = if (dr.flags.debits_must_not_exceed_credits)
                        dr.credits_posted
                    else
                        std.math.maxInt(u64);
                    const dr_available = dr_limit - dr.debits_posted - dr.debits_pending;
                    if (dr_available < amount) amount = dr_available;
                }

                if (t.flags.credits_at_most) {
                    const cr_limit = if (cr.flags.credits_must_not_exceed_debits)
                        cr.debits_posted
                    else
                        std.math.maxInt(u64);
                    const cr_available = cr_limit - cr.credits_posted - cr.credits_pending;
            ...
            if (t.flags.pending) {
                if (sum_overflows(amount, dr.debits_pending)) return .overflows_debits_pending;
                if (sum_overflows(amount, cr.credits_pending)) return .overflows_credits_pending;
            }
            if (sum_overflows(amount, dr.debits_posted)) return .overflows_debits_posted;
            if (sum_overflows(amount, cr.credits_posted)) return .overflows_credits_posted;
            // We assert that the sum of the pending and posted balances can never overflow:
            if (sum_overflows(amount, dr.debits_pending + dr.debits_posted)) {
                return .overflows_debits;
            }
            if (sum_overflows(amount, cr.credits_pending + cr.credits_posted)) {
                return .overflows_credits;
            }
            if (sum_overflows(t.timestamp, t.timeout)) return .overflows_timeout;

            if (dr.debits_exceed_credits(amount)) return .exceeds_credits;
            if (cr.credits_exceed_debits(amount)) return .exceeds_debits;
