...
pub fn run_in_docker(allocator: std.mem.Allocator, image: []const u8, cmds: []const u8) !void {
...
pub fn validate_docs(allocator: std.mem.Allocator, language: Docs) !void {
    var cmd = std.ArrayList(u8).init(allocator);
    try cmd.writer().print("mkdir /tmp/tests/; cd /tmp/tests/; echo $'{s}' > test.{s}; {s}; {s};", .{
        language.install_sample_file,
        language.extension,
        language.install_commands,
        language.install_sample_file_build_commands,
    });
...
pub fn main() !void {
    ...
    for (languages) |language| {
        ...
        try validate_docs(allocator, language);
